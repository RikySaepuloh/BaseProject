package id.riky.baseproject.utils
...
fun displayProgressBar(view:ProgressBar,isDisplayed:Boolean){
...
fun textViewFromHTML(view: TextView,text: String){
...
fun hideSoftKeyboard(iBinder: IBinder,context: Context) {
    val imm = context.getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
    imm.hideSoftInputFromWindow(iBinder, InputMethodManager.SHOW_IMPLICIT)
}
...
fun longDate2ddMMyyyy(value:String):String{
    val newdate: Date = SimpleDateFormat("dd-MM-yyyy HH:mm:ss", Locale.ENGLISH).parse(value)
    return SimpleDateFormat("dd-MM-yyyy", Locale.forLanguageTag("in_ID")).format(newdate)
}

fun yyyyMMdd2ddMMyyyy(value:String):String{
    val newdate: Date = SimpleDateFormat("yyyy/MM/dd", Locale.ENGLISH).parse(value)
    return SimpleDateFormat("dd-MM-yyyy", Locale.forLanguageTag("in_ID")).format(newdate)
}

fun longAnotherDate2ddMMyyyy(value:String):String{
    val newdate: Date = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).parse(value)
    return SimpleDateFormat("dd-MM-yyyy", Locale.forLanguageTag("in_ID")).format(newdate)
}

fun ddMMyyy2yyyyMMdd(value:String):String{
...
fun dateTodayyymmddslash(): String {
    return SimpleDateFormat("yyyy/MM/dd", Locale.getDefault()).format(Date())
}
